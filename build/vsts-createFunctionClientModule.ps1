...
$parentModule = 'VMDeploy.Guest'
if (-not $ModuleName) { $ModuleName = 'VMDeploy.Guest.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\VMDeploy.Guest'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\VMDeploy.Guest.psd1"
$prereqHash = @{
